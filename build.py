...
function personDelivery(mid){
  ...
  var m=S.m.find(function(x){return x.id===mid;});
  if(!m)return {decl:0,covered:0,unsold:0,notQuoted:0,util:null,projects:[],sellable:0};
  ...
  var res={decl:decl,covered:covered,unsold:decl-covered,notQuoted:notQ,sellable:sellable,
...
function personNote(m,billVsTgt,util,d){
  if(d.notQuoted>5)return _d0(d.notQuoted)+' days go to projects with nothing quoted on them.';
  if(billVsTgt<90)return 'Books '+(100-billVsTgt).toFixed(0)+'% less client time than the '+_p0(tgtOf('PL'+rPL(m.role)))+' target asks for.';
  if(util===null)return 'No quoted work to compare against yet.';
  if(util<70)return _d0(d.unsold)+' of their '+_d0(d.decl)+' client days are not backed by quoted work.';
  if(util>115)return 'Delivers '+(util-100).toFixed(0)+'% beyond the client time they owe: check the load.';
  if(util>=100&&billVsTgt>=100)return 'Fully deployed on quoted work, target met.';
  return _d0(d.unsold)+' days short of filling their client time.';
}
...
var ADMDEF={
  ...
  utilization:'Utilization rate = days backed by quoted work, divided by the client time the person should be filling (capacity x billability target). Billability says whether someone books enough client time; utilization says whether that time matches work that was actually quoted. Below 100% part of their client time has nothing quoted behind it.',
...
function admOverview(){
  ...
  var h='<div class="ucard"><div class="uct">Team at a glance</div>'
   ...
   +kpiCard({label:'Utilization',info:'utilization',value:util,scale:150,mark:100,
             ...
             verdict:function(v){return v>=100?{t:'work matches what was quoted',c:'#2f6e12'}:v>=85?{t:'slightly over budget',c:'#8a5a0c'}:{t:'more days than quoted',c:'#b32a1c'};}})
   +kpiCard({label:'Saturation',info:'saturation',value:sat,scale:150,mark:100,
             fmt:_p0,target:'target up to 100%',
             verdict:function(v){return v<=100?{t:'within capacity',c:'#2f6e12'}:v<=110?{t:'slightly over what we can deliver',c:'#8a5a0c'}:{t:'more quoted than deliverable',c:'#b32a1c'};}})
   +(comp.closed
  ...
  h+='</tbody></table>'
   +admLegend([['Billability','client days / capacity, against the target of each level'],['Utilization','days backed by quoted work / the client time they should fill'],['Saturation','quoted days / the days the team can bill: above 100% more is quoted than can be delivered']])+'</div>';
...
function admLevels(){
  var D=admData(),Dl=deliveryData();
  var tC=0,tB=0,tQ=0,tSell=0,tCov=0,tSellB=0,tN=0;
  var h='<div class="ucard"><div class="uct">By Price Level</div>'
   +'<div class="ucs">The three measures that matter, per level. Billability asks whether people book enough client time; utilization whether that time is backed by quoted work; saturation whether more has been quoted than the level can deliver.</div>'
   +weakDataNote()
   +'<table class="utbl"><thead><tr><th>Price Level</th><th class="r">People</th><th class="r">Capacity (days)</th>'
   +'<th class="r">Billability'+iHelp('billability')+'</th><th class="r">Utilization'+iHelp('utilization')+'</th>'
   +'<th class="r">Saturation'+iHelp('saturation')+'</th><th>Reading</th></tr></thead><tbody>';
  PLkeys.forEach(function(k){
    var cap=D.plCap[k],bill=D.plBill[k],bp=cap?bill/cap*100:0,tg=D.tgt[k],bvs=tg?bp/tg*100:0;
    var co=0,se=0;
    roster().forEach(function(m){if('PL'+rPL(m.role)!==k)return;var d=personDelivery(m.id);co+=d.covered;se+=d.sellable;});
    var u=se>0?co/se*100:null, sa=D.plSell[k]>0?D.quoted[k]/D.plSell[k]*100:null;
    tC+=cap;tB+=bill;tQ+=D.quoted[k];tSell+=D.plSell[k];tCov+=co;tSellB+=se;tN+=D.plN[k];
    var msg=(sa!==null&&sa>110)?'More quoted than the level can deliver'
      :(u!==null&&u<70)?'Much of their client time is not quoted'
      :(bvs<90)?'Books less client time than the target'
      :(sa!==null&&sa<60)?'Room to take on more work'
      :'In line';
    h+='<tr><td>'+light(u,100,70)+' <b>'+plLabel(k)+'</b><div style="font-size:10px;color:var(--t3);margin-left:18px">'+esc(plRanksTxt(D,k))+'</div></td>'
      +'<td class="r">'+D.plN[k]+'</td><td class="r">'+_d0(cap)+'</td>'
      +'<td class="r">'+_p0(bp)+'<div style="font-size:10px;color:var(--t3)">target '+_p0(tg)+' · '+vsTarget(bvs,100)+'</div></td>'
      +'<td class="r"><b style="color:'+lightCol(u,100,70)+'">'+(u===null?'-':_p0(u))+'</b></td>'
      +'<td class="r"><b style="color:'+lightCol(sa,100,110,true)+'">'+(sa===null?'-':_p0(sa))+'</b></td>'
      +'<td style="font-size:11.5px;color:var(--t2)">'+msg+'</td></tr>';
  });
  var bT=tC?tB/tC*100:0,uT=tSellB>0?tCov/tSellB*100:null,sT=tSell>0?tQ/tSell*100:null;
  h+='<tr class="tot"><td>Total</td><td class="r">'+tN+'</td><td class="r">'+_d0(tC)+'</td><td class="r">'+_p0(bT)+'</td>'
   +'<td class="r">'+(uT===null?'-':_p0(uT))+'</td><td class="r">'+(sT===null?'-':_p0(sT))+'</td><td></td></tr>';
  return h+'</tbody></table>'
   +admLegend([['Billability','client days / capacity, against the target of the level'],['Utilization','days backed by quoted work / the client time they should fill'],['Saturation','quoted days / the days the level can bill']])+'</div>';
}
...
function peopleWatch(kind,title){
  ...
  if(kind==='room'){                       // who a manager can still allocate
    pick=list.filter(function(x){return x.util!==null&&x.util<85||x.billVsTgt<90;})
             .sort(function(a,b){return (a.util===null?999:a.util)-(b.util===null?999:b.util);});
    intro='People whose client time is not fully backed by quoted work: the first place to look when staffing something new.';
  } else if(kind==='stretched'){           // who is already beyond their time
    pick=list.filter(function(x){return (x.util!==null&&x.util>115)||x.billVsTgt>115;})
             .sort(function(a,b){return (b.util||0)-(a.util||0);});
    intro='People delivering beyond the client time they owe. Adding work here has a cost.';
  } else {
    pick=list.filter(function(x){return x.band!=='ok'&&x.band!=='eff';})
             .sort(function(a,b){return (a.util===null?999:a.util)-(b.util===null?999:b.util);});
  ...
  h+='<div class="ucs">'+intro+'</div><table class="utbl"><thead><tr><th style="width:26px"></th><th>Name</th><th class="r">Days planned</th><th class="r">Of which quoted</th><th class="r">Utilization</th><th>What it says</th></tr></thead><tbody>';
  pick.forEach(function(x){
    h+='<tr style="cursor:pointer" onclick="S.aTab=\'people\';S.pBand=\'all\';S.pSel=\''+x.m.id+'\';R()">'
      ...
      +'<td class="r">'+_d0(x.decl)+'</td><td class="r">'+_d0(x.covered)+'</td>'
...
function projectsWatch(title,kind){
  ...
  if(kind==='needs'){
    pick=D.filter(function(r){return r.sold>0&&r.coverage!==null&&r.coverage<75;})
          .sort(function(a,b){return (b.sold-b.decl)-(a.sold-a.decl);});
    intro='Quoted work that nobody has planned yet: these need people.';
  } else if(kind==='over'){
    pick=D.filter(function(r){return r.coverage!==null&&r.coverage>125||r.sold<=0;})
          .sort(function(a,b){return (b.decl-b.sold)-(a.decl-a.sold);});
    intro='More days going in than were quoted, or work with nothing quoted at all.';
  } else {
    pick=D.map(function(r){return {r:r,b:projectBand(r)};}).filter(function(x){return x.b!=='eff'&&x.b!=='ok';})
          .sort(function(a,b){return Math.abs(b.r.decl-b.r.sold)-Math.abs(a.r.decl-a.r.sold);}).map(function(x){return x.r;});
...
function admDelivery(){
  ...
  var h='<div class="ucard"><div class="uct">Projects</div><div class="ucs">Days quoted on each project against the days the team plans to spend there. Open a row to see the detail per Price Level.</div>'
   +weakDataNote()
...
function admPeople(){
  ...
  var h=orphanCard()+leaverCard()+'<div class="ucard"><div class="uct">People</div><div class="ucs">Utilization first: whether the days each person reports are backed by days that were sold. Billability follows: whether they work on clients as much as their target expects.</div>'
   ...
   +'<th class="r">Quoted (days)</th><th class="r">Planned (days)</th><th class="r">Billability'+iHelp('billability')+'</th>'
  ...
  rows.forEach(function(x){
    ...
    if(sel){
      h+='<tr><td colspan="9" style="background:#f7f8fa;padding:0"><div style="padding:10px 14px">'
        ...
        +(x.projects.length?'<table class="utbl" style="background:transparent"><thead><tr><th>Project</th><th class="r">Days planned</th><th class="r">Of which quoted</th><th class="r">Not quoted</th></tr></thead><tbody>'
  ...
  return h+'</tbody></table>'
   +admLegend([['Utilization','days backed by quoted work / the client time they owe'],['Of which quoted','the part of their planned days that is backed by quoted work'],['Days planned','days they report they will spend'],['Billability','billable days / capacity, against their own target'],['Reading','how the two measures combine']])
